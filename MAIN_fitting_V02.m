...
%
%  Notes
%   - Version 02 - Data from Patuzzi/Sellick
...
load('data_patuzzi.mat')
zero_pad = [10,0;20,0];
Data01_18k = [zero_pad; Data01_18k];

handle = @IHC_DC;
fun  = @(a,xdata) IHC_DC(xdata,a);

guess = [Gmax u0 s0 u1 s1 Ga];  % starting guess
xdata = Data01_18k(:,1);      % X values
ydata = Data01_18k(:,2);          % Y values

lb = [0 0 0 0 0 0];
ub = [12e-9 inf inf inf inf inf];

%opts = statset('nlinfit');
%opts.FunValCheck = 'off';

%[param,~,~,~,~]   = nlinfit(xdata,ydata,fun,guess); 
param = lsqcurvefit(fun,guess,xdata,ydata,lb,ub);
...
grid on
xlabel('Levels in dB','FontSize',20) 
ylabel('DC component','FontSize',20)
set(gca, 'FontSize', axisnumbersize,  'fontweight', 'bold', 'linewidth', linesize);

% %% TEST
% 
% DC = IHC_DC(xdata,param);
% 
% figure
% plot(xdata, DC,'linewidth', linesize);
% hold on
% grid on
% legend('7kHz','18kHz')
% ylim([0 13])
...
G        = a(1)*1./((1+exp(-(x-a(2))/a(3))).*(1+exp(-(x-a(4))/a(5)))) + a(6) ; 
G_Somner = Gmax*1./((1+exp(-(x-u0)/s0)).*(1+exp(-(x-u1)/s1))) + Ga;
...
plot(x/1E-9, G_Somner/1E-9, '--', 'LineWidth', linesize);
legend('data','fitted','somner');
set(gca, 'FontSize', axisnumbersize,  'fontweight', 'bold', 'linewidth', linesize);
